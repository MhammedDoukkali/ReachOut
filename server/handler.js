...
const {username, passWord} = req.body ; 
  ...
  if(username.toLowerCase() === process.env.ADMIN_NAME.toLowerCase() && passWord===process.env.ADMIN_PASSWORD) {
    try {
      //connect to the database
      await client.connect();
      // if the sign in info matched add id to the data base to track security
      const id=uuidv4();
      await adminId.updateOne(
        { username },
        { $push: {id} }
     )
...
module.exports = { getinfoFromDb, addCenter, getQuotes,addDataFromEnv, getCenters, deleteCenter }